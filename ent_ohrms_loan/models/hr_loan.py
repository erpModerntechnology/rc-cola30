...
class HrLoan(models.Model):
    ...
    kafeel_1 = fields.Many2one(
        comodel_name='res.users',
        string="كفيل رقم 1",
        required=False)
    kafeel_2 = fields.Many2one(
        comodel_name='res.users',
        string="كفيل رقم 2",
        required=False)
    ...
    def action_submit(self):
        ...
        for rec in self.loan_lines:
            summ +=rec.amount
        if self.loan_amount  != summ:
            raise ValidationError(_('Loan Amount not Equal summition of lines'))
...
class InstallmentLine(models.Model):
    ...
    def onchange_amount(self):
        total_amount = self.loan_id.loan_amount
        lines_total = sum(self.search([('loan_id', '=', self.loan_id.id)]).mapped('amount'))
        if lines_total !=  total_amount:
            raise ValidationError(
                _("You entered lines with total amount {} but the total loan value is {}".format(lines_total,
                                                                                                 total_amount)))
