...
class HrEmployee(models.Model):
    ...
    def get_late_check_in_count(self):
        for rec in self :
            late_check_in_count = self.env['late.check_in'].search_count([('employee_id', '=', rec.id)])
            print(late_check_in_count)
            if late_check_in_count:
                rec.late_check_in_count = late_check_in_count
            else:
                rec.late_check_in_count = 0
